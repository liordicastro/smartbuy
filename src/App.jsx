...
// --- 2. מודאל השוואת מוצרים עם המלצת מומחים ---
    ...
    useEffect(() => {
        if (!document.getElementById('clean-translate')) {
            ...
            style.innerHTML = `
                ...
                /* הסתרת פס הגלילה לכרטיסיות אך השארת האפשרות לגלול */
    ...
    return (
        <HelmetProvider>
            <div className="min-h-screen bg-gray-50 text-right font-assistant overflow-x-hidden" dir="rtl">
                ...
                {/* סרגל עליון (Top Bar) לסינון וטיפים */}
                    ...
                    <div className="flex flex-col lg:flex-row gap-6 mb-8">
                        ...
                        <div className="bg-white p-6 rounded-[30px] shadow-sm border flex-grow flex flex-col md:flex-row gap-8 items-center z-10 relative">
                            ...
                            <div className="w-full md:w-2/3">
                                <label className="block text-xs font-black text-gray-400 mb-2">סינון לפי מותגים נבחרים</label>
                                <div className="flex flex-wrap gap-2">
                                    {availableBrands.map(b => (
                                        <button 
                                            key={b} 
                                            onClick={() => toggleBrand(b)}
                                            className={`px-4 py-2 rounded-full text-[10px] font-bold border transition-all ${selectedBrands.includes(b) ? 'bg-[#1e3a8a] text-white border-[#1e3a8a]' : 'bg-gray-50 text-gray-600 hover:border-[#1e3a8a]'}`}
                                        >
                                            {b}
                                        </button>
                                    ))}
                        ...
                        <div className="bg-gradient-to-br from-[#1e3a8a] to-[#152a63] p-6 rounded-[30px] text-white shadow-md border-b-4 border-[#FFD814] w-full lg:w-1/3 flex flex-col justify-center">
                            ...
                            <p className="text-xs leading-relaxed font-bold opacity-90">מוצרי חשמל חכמים עם טכנולוגיית Inverter יכולים לחסוך עד 30% בהוצאות החשמל הביתית שלכם לאורך זמן.</p>
                        </div>
                    </div>
                </div>
                ...
                <main className="max-w-7xl mx-auto p-4 md:p-8 pt-0 space-y-16">
                    {filter === "All" && !searchQuery && selectedBrands.length === 0 ? (
                        /* דף הבית: שורות מכווצות (נכנסים יותר מוצרים אופקית) */
                        Object.keys(categorizedGroups).map(catKey => (
                            <section key={catKey}>
                                ...
                                {/* Flex-shrink-0 כדי שהרוחב יישמר, ו-w-full לגלילה */}
                                <div className="flex overflow-x-auto gap-4 pb-6 hide-scroll snap-x scroll-smooth w-full">
                                    {categorizedGroups[catKey].map(p => (
                                        <div key={p.id} className="w-[260px] min-w-[260px] max-w-[260px] flex-none snap-start bg-white p-5 rounded-3xl shadow-sm border relative hover:shadow-xl transition-all flex flex-col justify-between group">
                        ...
                        /* דפי קטגוריות / סינון: GRID */
                        <section>
                            <div className="flex items-center justify-between mb-8 bg-white p-4 rounded-2xl border shadow-sm">
                                 <h2 className="text-xl font-black text-[#1e3a8a]">{filter === "All" ? "תוצאות סינון" : categoryMap[filter]}</h2>
                ...
                <footer className="bg-[#1e3a8a] text-white py-16 px-6 border-t-8 border-[#FFD814] mt-20">
                    <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10">
                        ...
                        <div>
                            <h4 className="font-black text-lg mb-4 border-b-2 border-[#FFD814] pb-2 inline-block">תשלום בטוח</h4>
                ...
                {/* Cart Drawer - ללא טשטוש מסך מעצבן ועם X במקום פלוס */}
                <div className={`fixed top-0 right-0 h-full w-80 md:w-[400px] bg-white shadow-2xl z-[500] transition-transform duration-500 border-l-8 border-[#1e3a8a] ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                    ...
                    <div className="p-4 overflow-y-auto h-[60vh] space-y-4 bg-gray-50 custom-scrollbar">
                        {cart.length === 0 ? (
                            ...
                            cart.map((item, i) => (
                                <div key={i} className="flex gap-4 bg-white p-4 rounded-2xl border shadow-sm relative">
                                    <button onClick={() => setCart(cart.filter((_, idx)=>idx!==i))} className="absolute top-2 right-2 text-red-500 hover:text-red-700 bg-red-50 w-6 h-6 rounded-full flex items-center justify-center font-bold"><i className="fa-solid fa-xmark text-xs"></i></button>
